...
class TradingMonitorGUI:
    def __init__(self, root):
        ...
        self.option_codes = [] # 已成交合约

        self.n = 0.08 # 信号阈值
        # self.alarm_price = 8400
        self.contract_price = 70.5 # 合约成交价格
        self.alarm_price = {} # 合约成交时标的的价格 * 预警系数
    ...
    def init_signal_stats(self, contract, dict_history):
        """用今日历史数据初始化每个合约的pos/neg统计"""
        stats = {}
        start = datetime.now().strftime('%Y%m%d') + '090000'
        today = datetime.now().date().strftime('%Y%m%d')
        hist_data = self.get_market_data(contract, '1m', start, today, -1)
